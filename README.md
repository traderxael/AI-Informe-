...
- RSS públicos (labs, medios, Google News multi-país, HN, Reddit con degradación suave). 29 feeds vivos, auditados con `scripts/auditar_feeds.py`.
...
- **Señales de X (Twitter)**: `scripts/fx_viral.py` monitorea 16 cuentas + 4 búsquedas de nicho vía FxTwitter API → `datos/fx_viral_YYYY-MM-DD.json` (top-40 por views, agrupado por tema). Es también la cola de contenido para clips de AutoClip.
...
scripts/
  ...
  test_pipeline.py        # tests sin red (14 tests)
...
```bash
python scripts/test_pipeline.py     # 14 tests, sin red
...
python scripts/auditar_feeds.py      # cobertura real de los 29 feeds (cuántos items trae cada uno)
```
...
Cron `0 12 * * *` (09:00 Chile) o manual con `workflow_dispatch`. Pasos, todos con `continue-on-error` donde corresponde:
...
6. **Señales de X (FxTwitter)** — 16 cuentas + 4 búsquedas de nicho
